...
export default function App() {
  const { cartons, addCarton, updateCarton, deleteCarton, cycleStatus, nextNumber, setCartons } = useCartons()
  const { fetchFromGist, pushToGist, savePat, clearPat, hasPat, syncing, lastSync, syncError } = useGistSync()
  ...
  function handleSave(data) {
    ...
    else updateCarton(modal.carton.id, data)
    setModal(null)
  }

  function handleSavePat(pat) {
    savePat(pat)
    setPatStored(true)
  }
  ...
  const suggestedNumber = modal === 'add' ? nextNumber('B', cartons) : undefined
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col max-w-lg mx-auto">
      ...
      {/* FAB — caché sur l'onglet réglages */}
      {tab !== 'settings' && (
        <button
